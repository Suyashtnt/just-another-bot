...
export default class ServerInfoCommand extends Command {
	...
	async exec(msg: Message) {
		...
		embed.addField("General Info", `${guild.owner ? `Owner: ${guild.owner.displayName}(${guild.owner.user.username}${guild.owner.user.discriminator})` : ''}
		ID: ${guild.id}
		...
		embed.addField("Boost Info", `Boost Level: ${guild.premiumTier}
		Boosts: ${guild.premiumSubscriptionCount ? guild.premiumSubscriptionCount : "no boosts"}
		...
		try {
			...
			embed.addField("Stats", `Members: ${members.filter(val => !val.user.bot).size}
